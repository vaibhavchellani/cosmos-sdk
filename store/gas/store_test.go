...
func newGasTank(limit types.Gas) *types.GasTank {
	return types.NewGasTank(types.NewGasMeter(limit), types.DefaultGasConfig())
}
...
func TestGasKVStoreBasic(t *testing.T) {
	...
	require.Empty(t, st.Get(utils.KeyFmt(1)), "Expected `key1` to be empty")
	require.Equal(t, tank.GasConsumed(), types.Gas(183))
}
...
func TestGasKVStoreIterator(t *testing.T) {
	...
	require.Equal(t, tank.GasConsumed(), types.Gas(356))
}
